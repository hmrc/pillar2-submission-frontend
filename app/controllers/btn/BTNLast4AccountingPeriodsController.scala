...
class BTNLast4AccountingPeriodsController @Inject() (
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(BTNLast4AccountingPeriodsPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(BTNLast4AccountingPeriodsPage, mode, updatedAnswers))
      )
  }
